...
function main(content = null) {
    ...
    if (content != null) {
        ...
        // Run the auto-focus code when UI.main() runs with a different template.
        // We can detect this with strings, which is typed TemplateStringsArray,
        // and is stable for a given lit-html template.
        if (overlay == null && content.strings !== prev_main) {
            autoFocus(main_el, true);
            prev_main = content.strings;
        }
    }
}
...
async function handleSubmit(e, dlg) {
    try {
        ...
        // Validate forms (if any)
        {
            ...
            let map = form_maps.get(dlg.update);
            let contexts = map?.values?.() ?? [];

            for (let ctx of contexts) {
                for (let widget of ctx.model.widgets) {
                    if (widget.key == null)
                        continue;
...
function autoFocus(el, force = false) {
    setTimeout(() => {
        let widget0 = el.querySelector('label > *[autofocus]') ??
                      el.querySelector(`label > input[type=text]:not(:disabled),
                                        label > input[type=password]:not(:disabled),
...
function form(render, obj, func) {
    let map = form_maps.get(render);

    if (map == null) {
        map = new Map;
        form_maps.set(render, map);
    }
    ...
    if (ctx == null) {
        ctx = {
            run: () => {
                ...
                let builder = new FormBuilder(ctx.state, ctx.model, { wrap: 'label' });
                func(builder);
            },
    ...
    let widgets = ctx.model.widgets.map(widget => widget.render());
    return widgets;
}
