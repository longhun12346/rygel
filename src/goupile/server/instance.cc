...
bool MigrateInstance(sq_Database *db, int target)
{
    ...
    bool success = db->Transaction([&]() {
        ...
        switch (version) {
            ...
            case 134: {
                bool success = db->RunMany(R"(
                    ALTER TABLE rec_exports ADD COLUMN secret TEXT;
                )");
                if (!success)
                    return false;
            } [[fallthrough]];
            ...
            case 141: {
                bool success = db->RunMany(R"(
                    UPDATE rec_threads SET hid = sequence;
                    ALTER TABLE rec_fragments ADD COLUMN hid TEXT;
                )");
                if (!success)
                    return false;
            } [[fallthrough]];
            ...
            case 142: {
                bool success = db->RunMany(R"(
                    CREATE INDEX rec_threads_s ON rec_threads (sequence);
                    CREATE INDEX rec_threads_h ON rec_threads (hid);
                )");
                if (!success)
                    return false;
            } [[fallthrough]];
            ...
            case 143: {
                bool success = db->RunMany(R"(
                    DROP INDEX rec_threads_s;
                    DROP INDEX rec_threads_h;

                    ALTER TABLE rec_threads RENAME COLUMN hid TO hid_BAK;
                    ALTER TABLE rec_threads ADD COLUMN hid TEXT COLLATE natsort;
                    CREATE INDEX rec_threads_h ON rec_threads (hid);

                    UPDATE rec_threads SET hid = hid_BAK;

                    ALTER TABLE rec_threads DROP COLUMN hid_BAK;
                )");
                if (!success)
                    return false;
            } [[fallthrough]];
            ...
            case 144: {
                bool success = db->RunMany(R"(
                    ALTER TABLE rec_threads ADD COLUMN fid TEXT;
                    CREATE UNIQUE INDEX rec_threads_f ON rec_threads (fid);
                )");
                if (!success)
                    return false;
            } [[fallthrough]];
