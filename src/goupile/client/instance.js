    ...
    try {
        ...
        return async api => {
            ...
            try {
                ...
                await throwRunError(err, build.map);
            }
        };
    } catch (err) {
        await throwRunError(err, build.map);
...
async function throwRunError(err, map) {
    let pos = Util.parseEvalErrorPos(err);
    let line = await bundler.resolve(map, pos.line, pos.column);
